<%@ page contentType="text/html;charset=UTF-8" language="java" %>

<div class="container2">
    <ul class="mid">
        <li>
            <a href="/offers" class="ads-p">Обьявления</a>

            <a href="/offers">
                <div class="ads-img"></div>
            </a>
            <ul class="dropUl">
                <li><a href="/offers">Топ объявлений</a></li>
                <li><a href="/create-offer">Создать объявление</a></li>
            </ul>
        </li>
        <li>
            <a href="/tenders" class="ads-p">Тендеры</a>
            <a href="/tenders">
                <div class="ads-img2"></div>
            </a>
            <ul class="dropUl">
                <li><a href="/tender-make">Создать тендер</a></li>
                <li><a href="/doer-create">Стать исполнителем</a></li>
            </ul>
        </li>
        <li>
            <a href="/project/list" class="ads-p">Проекты</a>
            <a href="/project/list">
                <div class="ads-img3"></div>
            </a>
            <ul class="dropUl">
                <li><a href="/project/list?type=renovation">Реструкторизация</a></li>
                <li><a href="/project/list?type=prototype">Готовый прототип</a></li>
                <li><a href="/project/list?type=project_on_paper">Проект на бумаге</a></li>
                <li><a href="/project/list?type=know_how">Ноу-Хау</a></li>
            </ul>
        </li>
        <li>
            <a href="/blog-post/news?pageNumber=0" class="ads-p">Новости</a>

            <a href="/blog-post/news?pageNumber=0">
                <div class="ads-img4"></div>
            </a>

            <ul class="dropUl">
                <li><a href="/blog-post/news?pageNumber=0">Топ новостей</a></li>
                <li><a href="/blog-create">Создать новостной блог</a></li>
            </ul>
        </li>
        <li>
            <div class="slider">
                <ul id="services-menu-bxslider" class="bxslider">
                    <li><img src="/resources/images/elektronnye-torgi-kto-30-04-2015.png" /></li>
                    <li><img src="/resources/images/elektronnye-torgi-kto-30-04-2015.png" /></li>
                    <li><img src="/resources/images/elektronnye-torgi-kto-30-04-2015.png" /></li>
                </ul>
            </div>
        </li>
    </ul>
</div>